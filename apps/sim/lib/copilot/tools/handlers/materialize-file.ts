...
async function executeSave(
  ...
  if (isArchiveFileName(displayName)) {
    // Saving would move the row out of the chat-scoped uploads namespace — the
    // only place extract can resolve it — permanently stranding the contents
    // behind a binary placeholder under files/.
    return {
      success: false,
