...
Copilotはエディター内のアシスタントで、AACFlow Copilotを使用してワークフローの構築や編集を支援し、それらを理解して改善するのに役立ちます。以下のことができます：

- **説明**: AACFlowや現在のワークフローに関する質問に回答
- **ガイド**: 編集の提案やベストプラクティスの提示
- **編集**: 承認後にブロック、接続、設定の変更を実行

<Callout type="info">
  CopilotはAACFlowが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（aacflow.io → 設定 → Copilot）でCopilot APIキーを生成してください
  1. [aacflow.io](https://aacflow.io) → 設定 → Copilotに移動し、Copilot APIキーを生成
