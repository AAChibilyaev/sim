...
AACFlowはすべてのワークフロー実行のコストを自動的に計算し、AIモデルの使用量と実行料金に基づいた透明な価格設定を提供します。これらのコストを理解することで、ワークフローを最適化し、予算を効果的に管理することができます。
...
<Tabs items={['ホステッドモデル', '独自のAPIキーを使用']}>
  <Tab>
    **ホステッドモデル** - AACFlowは、エージェントブロック用に1.4倍の価格乗数を適用したAPIキーを提供します:
...
ワークフローがAACFlowのホスティングAPIキーを使用するツールブロックを利用する場合、操作ごとに料金が発生します。BYOKで独自のキーを使用すると、プロバイダーに直接支払うことができます。
...
AACFlowは**基本サブスクリプション + 超過料金**の課金モデルを採用しています。
...
未請求の超過料金が$50に達すると、AACFlowは未請求金額の全額を自動的に請求します。
...
- [設定 → サブスクリプション](https://aacflow.io/settings/subscription)で現在の使用状況を確認する
...
未請求の超過分が$50に達すると、AACFlowは自動的に未請求金額の全額を請求します。
...
- [設定 → サブスクリプション](https://aacflow.io/settings/subscription)で現在の使用状況を確認してください
