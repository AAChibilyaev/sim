...
AACFlow bietet eine umfassende externe API zum Abfragen von Workflow-Ausführungsprotokollen und zum Einrichten von Webhooks für Echtzeit-Benachrichtigungen, wenn Workflows abgeschlossen werden.
...
Sie können API-Schlüssel in Ihren Benutzereinstellungen im AACFlow-Dashboard generieren.
...
Wenn eine Workflow-Ausführung abgeschlossen ist, sendet AACFlow die folgende Payload (über Webhook POST, E-Mail oder Slack):
...
Wenn Sie ein Webhook-Geheimnis konfigurieren, überprüfen Sie die Signatur, um sicherzustellen, dass der Webhook von AACFlow stammt:
...
2. **Webhook-Sicherheit**: Konfiguriere immer ein Webhook-Secret und überprüfe Signaturen, um sicherzustellen, dass Anfragen von AACFlow stammen.
