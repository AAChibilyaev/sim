...
Comprendre comment les workflows s'exécutent dans AACFlow est essentiel pour créer des automatisations efficaces et fiables. Le moteur d'exécution gère automatiquement les dépendances, la concurrence et le flux de données pour garantir que vos workflows fonctionnent de manière fluide et prévisible.

## Comment les workflows s'exécutent

Le moteur d'exécution de AACFlow traite les workflows de manière intelligente en analysant les dépendances et en exécutant les blocs dans l'ordre le plus efficace possible.
...
AACFlow fournit différents types de blocs qui servent à des fins spécifiques dans vos flux de travail :
...
Lorsque les workflows s'exécutent, AACFlow offre une visibilité en temps réel sur le processus d'exécution :
