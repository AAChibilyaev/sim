...
AACFlow calcule automatiquement les coûts pour toutes les exécutions de flux de travail, offrant une tarification transparente basée sur l'utilisation des modèles d'IA et les frais d'exécution. Comprendre ces coûts vous aide à optimiser les flux de travail et à gérer efficacement votre budget.
...
<Tabs items={['Modèles hébergés', 'Apportez votre propre clé API']}>
  <Tab>
    **Modèles hébergés** - AACFlow fournit des clés API avec un multiplicateur de prix de 1,4x pour les blocs Agent :
...
Lorsque les workflows utilisent des blocs d'outils avec les clés API hébergées par AACFlow, les coûts sont facturés par opération. Utilisez vos propres clés via BYOK pour payer directement les fournisseurs.
...
AACFlow utilise un modèle de facturation **abonnement de base + dépassement** :
...
Lorsque le dépassement non facturé atteint 50 $, AACFlow facture automatiquement le montant total non facturé.
...
- Consultez votre utilisation actuelle dans [Paramètres → Abonnement](https://aacflow.io/settings/subscription)
