...
AACFlow fournit une API externe complète pour interroger les journaux d'exécution des workflows et configurer des webhooks pour des notifications en temps réel lorsque les workflows sont terminés.
...
Vous pouvez générer des clés API depuis vos paramètres utilisateur dans le tableau de bord AACFlow.
...
Lorsqu'une exécution de workflow se termine, AACFlow envoie la charge utile suivante (via webhook POST, e-mail ou Slack) :
...
Si vous configurez un secret webhook, vérifiez la signature pour vous assurer que le webhook provient de AACFlow :
...
2. **Sécurité des webhooks** : Configurez toujours un secret de webhook et vérifiez les signatures pour vous assurer que les requêtes proviennent de AACFlow.
