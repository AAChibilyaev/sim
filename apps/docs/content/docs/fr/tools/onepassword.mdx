...
L'[API 1Password Connect](https://developer.1password.com/docs/connect/) permet un accès programmatique aux coffres et aux éléments au sein du compte 1Password d'une organisation. Cette intégration dans AACFlow vous permet d'automatiser la récupération de secrets, les flux d'intégration, la rotation de secrets, les audits de coffres, et bien plus encore, le tout de manière sécurisée et auditable.

Avec 1Password dans votre flux de travail AACFlow, vous pouvez :
