---
title: Aperçu
description: Les déclencheurs sont les moyens essentiels pour lancer les flux de travail AACFlow
---
...
Lorsque vous cliquez sur **Exécuter** dans l'éditeur, AACFlow sélectionne automatiquement quel déclencheur exécuter selon l'ordre de priorité suivant :
...
**Déclencheurs externes avec charges utiles simulées** : lorsque des déclencheurs externes (webhooks et intégrations) sont exécutés manuellement, AACFlow génère automatiquement des charges utiles simulées basées sur la structure de données attendue du déclencheur. Cela garantit que les blocs en aval peuvent résoudre correctement les variables pendant les tests.
...
Les personnes invitées reçoivent un e-mail avec un lien pour connecter leur compte. Une fois connectée, leur boîte de réception est automatiquement incluse dans le groupe de surveillance. Les personnes invitées n'ont pas besoin d'être membres de votre organisation AACFlow.
