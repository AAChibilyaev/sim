...
AACFlow автоматически рассчитывает стоимость каждого запуска рабочего процесса на основе использования ИИ-моделей и базовой платы за запуск.

## Кредиты

AACFlow использует **кредиты** как единицу измерения для всего использования. **1 кредит = $0.005**.

Все лимиты тарифов, счётчики использования и пороги биллинга отображаются в кредитах во всём интерфейсе AACFlow. Суммы в долларах в этой документации приведены для справки.
...
<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** — AACFlow предоставляет ключи API с множителем цены 1.1x для блоков Agent:
...
Когда рабочие процессы используют блоки инструментов с размещёнными ключами API AACFlow, стоимость взимается за операцию. Используйте собственные ключи через BYOK для оплаты провайдерам напрямую.
...
Используйте собственные ключи API для поддерживаемых провайдеров вместо размещённых ключей AACFlow, чтобы платить по базовым ценам без наценки.
...
При настройке рабочие процессы используют ваш ключ вместо размещённых ключей AACFlow. При удалении рабочие процессы автоматически возвращаются к размещённым ключам с множителем.
...
AACFlow has two paid plan tiers - **Pro** and **Max**. Either can be used individually or with a team. Team plans pool credits across all seats in the organization.
...
AACFlow uses a **base subscription + overage** billing model:
...
When on-demand is enabled and unbilled overage reaches $100, AACFlow automatically bills the full unbilled amount.
...
- Review your current usage in [Settings → Subscription](https://aacflow.io/settings/subscription)
...
<FAQ items={[
  ...
  { question: "What is the credit-to-dollar conversion rate?", answer: "1 credit equals $0.005. All plan limits, usage meters, and billing thresholds in the AACFlow UI are displayed in credits." },
  ...
  { question: "How does the 1.1x hosted model multiplier work?", answer: "When you use AACFlow's hosted API keys (instead of bringing your own), a 1.1x multiplier is applied to the base model pricing for Agent blocks. This covers infrastructure and API management costs. You can avoid this multiplier by using your own API keys via the BYOK feature." },
  ...
  { question: "When does threshold billing trigger?", answer: "When on-demand billing is enabled and your unbilled overage reaches $100, AACFlow automatically bills the full unbilled amount. This spreads large charges throughout the month instead of accumulating one large bill at period end." },
]} />
