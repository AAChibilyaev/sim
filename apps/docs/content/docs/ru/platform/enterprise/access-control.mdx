...
AACFlow resolves the governing group for a user in a workspace deterministically:
...
When a user runs a workflow or uses Mothership, AACFlow reads the resolved group's configuration and applies it:
...
<Image src="/static/enterprise/access-control-model-providers.png" alt="Model Providers tab showing a grid of AI providers including Ollama, vLLM, OpenAI, Anthropic, Google, Azure OpenAI, and others with checkboxes to allow or restrict access" width={900} height={500} /> The list shows all providers available in AACFlow.
...
| API Keys | Hides the AACFlow Keys tab in Settings |
...
| Skills | Disables the use of AACFlow Skills in workflows and agents |
...
| Feature | Effect when checked |
|---------|-------------------|
| AACFlow Mailer | Hides the AACFlow Mailer (Inbox) feature |
...
Once enabled, permission groups are managed through **Settings → Enterprise → Access Control** the same way as AACFlow Cloud.
