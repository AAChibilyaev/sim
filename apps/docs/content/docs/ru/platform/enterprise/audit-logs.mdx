...
Audit logs are also accessible through the AACFlow API for integration with external SIEM or log management tools.
...
<FAQ items={[
  {
    ...
    answer: "Organization owners and admins can view audit logs. On AACFlow Cloud, you must be on the Enterprise plan."
  },
  {
    question: "Are audit logs tamper-proof?",
    answer: "Audit log entries are append-only and cannot be modified or deleted through the AACFlow interface or API. They represent a reliable record of actions taken in your organization."
  },
  {
