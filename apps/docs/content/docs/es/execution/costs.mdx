...
AACFlow calcula automáticamente los costos de todas las ejecuciones de flujos de trabajo, proporcionando precios transparentes basados en el uso de modelos de IA y cargos de ejecución. Entender estos costos te ayuda a optimizar los flujos de trabajo y gestionar tu presupuesto de manera efectiva.
...
<Tabs items={['Modelos alojados', 'Trae tu propia clave API']}>
  <Tab>
    **Modelos alojados** - AACFlow proporciona claves API con un multiplicador de precios de 1.4x para bloques de agente:
...
Cuando los flujos de trabajo usan bloques de herramientas con las claves API alojadas de AACFlow, los costos se cobran por operación. Usa tus propias claves a través de BYOK para pagar directamente a los proveedores.
...
AACFlow utiliza un modelo de facturación de **suscripción base + excedente**:
...
Cuando el excedente no facturado alcanza los $50, AACFlow factura automáticamente el monto total no facturado.
...
- Revisa tu uso actual en [Configuración → Suscripción](https://aacflow.io/settings/subscription)
...
Cuando el excedente no facturado alcanza $50, AACFlow factura automáticamente el monto total no facturado.
