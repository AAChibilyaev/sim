...
AACFlow proporciona una API externa completa para consultar registros de ejecución de flujos de trabajo y configurar webhooks para notificaciones en tiempo real cuando los flujos de trabajo se completan.
...
Puedes generar claves de API desde la configuración de usuario en el panel de control de AACFlow.
...
Cuando se completa la ejecución de un flujo de trabajo, AACFlow envía la siguiente carga útil (vía webhook POST, correo electrónico o Slack):
...
Si configuras un secreto de webhook, verifica la firma para asegurar que el webhook proviene de AACFlow:
...
2. **Seguridad de webhook**: Siempre configura un secreto de webhook y verifica las firmas para asegurar que las solicitudes provienen de AACFlow.
