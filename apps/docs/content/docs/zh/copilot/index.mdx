...
Copilot 是您编辑器中的助手，可帮助您使用 AACFlow Copilot 构建和编辑工作流，同时理解和改进它们。它可以：

- **解释**：回答有关 AACFlow 和您当前工作流的问题
- **指导**：建议编辑和最佳实践
- **编辑**：在您批准后对模块、连接和设置进行更改

<Callout type="info">
  Copilot 是一个由 AACFlow 管理的服务。对于自托管部署，请在托管应用程序中生成一个 Copilot API 密钥（aacflow.io → 设置 → Copilot）
  1. 前往 [aacflow.io](https://aacflow.io) → 设置 → Copilot 并生成一个 Copilot API 密钥
