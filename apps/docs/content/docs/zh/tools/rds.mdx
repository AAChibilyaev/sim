...
在 AACFlow 中，RDS 集成使您的代理能够以安全且编程的方式与 Amazon Aurora Serverless 数据库交互。支持的操作包括：
...
此集成允许您的代理在无需人工干预的情况下自动化各种数据库操作。通过将 AACFlow 与 Amazon RDS 连接，您可以构建能够在工作流中管理、更新和检索关系数据的代理——无需处理数据库基础设施或连接。
{/* MANUAL-CONTENT-END */}
