...
在 AACFlow 中，Qdrant 集成使您的代理能够以编程方式将 Qdrant 作为其工作流程的一部分进行交互。支持的操作包括：

- **插入或更新（Upsert）**：在 Qdrant 集合中插入或更新点（向量和负载）
- **搜索（Search）**：执行相似性搜索，以找到与给定查询向量最相似的向量，并可选择过滤和自定义结果
- **获取（Fetch）**：通过 ID 从集合中检索特定点，并可选择包括负载和向量

此集成使您的代理能够利用强大的向量搜索和管理功能，从而实现高级自动化场景，例如语义搜索、推荐和上下文检索。通过将 AACFlow 与 Qdrant 连接，您可以构建能够理解上下文、从大型数据集中检索相关信息并提供更智能、更个性化响应的代理——而无需管理复杂的基础设施。
{/* MANUAL-CONTENT-END */}
