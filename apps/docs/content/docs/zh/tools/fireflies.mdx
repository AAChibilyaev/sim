...
[Fireflies.ai](https://fireflies.ai/) 是一个会议转录与智能平台，可与 AACFlow 集成，让你的代理可以通过零代码自动化，直接处理会议录音、转录和洞察。

Fireflies 在 AACFlow 中的集成提供了以下工具：
...
- **转录完成时触发工作流：** 使用提供的 webhook 触发器，在 Fireflies 会议转录完成后自动激活 AACFlow 工作流，实现基于新会议数据的实时自动化和通知。
