...
[Kalshi](https://kalshi.com) 是一个联邦监管的交易所，用户可以直接交易未来事件结果的预测市场。Kalshi 强大的 API 和 AACFlow 集成使代理和工作流能够以编程方式访问平台的各个方面，支持从研究和分析到自动化交易和监控的所有内容。

通过 Kalshi 在 AACFlow 中的集成，您可以：
