...
以下 AACFlow SSH 工具使您的代理能够作为更大自动化的一部分与服务器交互：

- `ssh_execute_command`：远程运行任何单个 shell 命令并捕获输出、状态和错误。
- `ssh_execute_script`：在远程系统上上传并执行完整的多行脚本。
- （即将推出其他工具，例如文件传输。）

通过将 SSH 集成到您的代理工作流中，您可以自动化安全访问、远程操作和服务器编排——简化 DevOps、IT 自动化和自定义远程管理，所有操作均可在 AACFlow 中完成。
{/* MANUAL-CONTENT-END */}
