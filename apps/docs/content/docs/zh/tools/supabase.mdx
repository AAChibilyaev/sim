...
**在 AACFlow 中使用 Supabase**

AACFlow 的 Supabase 集成使您能够轻松地将代理工作流连接到您的 Supabase 项目。只需填写几个配置字段——您的项目 ID、表名和服务角色密钥，您就可以直接从 AACFlow 模块中安全地与数据库交互。该集成简化了 API 调用的复杂性，让您专注于构建逻辑和自动化流程。

**在 AACFlow 中使用 Supabase 的主要优势：**
...
无论您是在构建内部工具、自动化业务流程，还是支持生产应用程序，AACFlow 中的 Supabase 都为您提供了一种快速、可靠且对开发者友好的方式来管理数据和后端逻辑——无需基础设施管理。只需配置模块，选择所需的操作，其余的交给 AACFlow 处理。
{/* MANUAL-CONTENT-END */}
