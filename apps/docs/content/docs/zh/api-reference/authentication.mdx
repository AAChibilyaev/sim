...
To access the AACFlow API, you need an API key. AACFlow supports two types of API keys — **personal keys** and **workspace keys** — each with different billing and access behaviors.
...
To generate a personal key, open **Account settings** → **AACFlow API keys**. Workspace
administrators can create shared keys from **Workspace settings** → **AACFlow API keys**.
