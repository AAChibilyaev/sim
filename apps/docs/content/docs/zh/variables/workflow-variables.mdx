...
AACFlow 中的变量充当数据的全局存储，可以被工作流中的任何模块访问和修改，从而允许您通过全局变量在整个工作流中存储和共享数据。它们提供了一种强大的方式来在工作流的不同部分之间共享信息、维护状态并创建更动态的应用程序。
