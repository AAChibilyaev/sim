...
AACFlow 提供了一个全面的外部 API，用于查询工作流执行日志，并在工作流完成时设置实时通知的 webhook。
...
您可以在 AACFlow 仪表板的用户设置中生成 API 密钥。
...
当工作流执行完成时，AACFlow 会发送以下负载（通过 webhook POST、电子邮件或 Slack）：
...
如果您配置了 webhook 密钥，请验证签名以确保 webhook 来自 AACFlow：
...
2. **Webhook 安全性**：始终配置一个 webhook 密钥并验证签名，以确保请求来自 AACFlow。
