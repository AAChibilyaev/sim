...
| Field | Description |
|-------|-------------|
| **URL** | Slug that forms the public URL, e.g. `https://aacflow.io/chat/your-slug`. Lowercase letters, numbers, and hyphens only. Must be unique across all workspaces. |
...
<Image src="/static/chat/chat-deploy-access-email.png" alt="Access control section with Email tab selected, showing an Allowed emails field with @aacflow.io domain added" width={800} height={300} />
...
```
https://aacflow.io/chat/your-slug
```
...
```html
<iframe
  src="https://aacflow.io/chat/your-slug"
  width="100%"
  height="600"
  frameborder="0"
  title="Chat"
...
<Tabs items={['cURL', 'TypeScript']}>
  <Tab value="cURL">
```bash
curl -X POST https://aacflow.io/api/chat/your-slug \
...
```typescript
const response = await fetch('https://aacflow.io/api/chat/your-slug', {
  method: 'POST',
...
curl -X POST https://aacflow.io/api/chat/your-slug/otp \
  -H "Content-Type: application/json" \
  -d '{ "email": "allowed@example.com" }'

# Step 2: Verify OTP — save the Set-Cookie header for subsequent requests
curl -X PUT https://aacflow.io/api/chat/your-slug/otp \
  -H "Content-Type: application/json" \
  -c cookies.txt \
...
**Chat not loading in iframe** — Check your site's Content Security Policy allows iframes from `aacflow.io`.
...
<FAQ items={[
  ...
  { question: "How does email OTP authentication work?", answer: "When a user opens an email-protected chat, they enter their email address. If it matches the allowed list, AACFlow sends a 6-digit OTP to that address. The user enters the code, and a session cookie is set for the duration of their visit." },
