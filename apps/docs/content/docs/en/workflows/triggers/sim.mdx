---
title: AACFlow Workspace Events
---
...
<ul className="list-disc space-y-1 pl-6">
  <li>The workflow containing the AACFlow Workspace Events trigger must be <strong>deployed</strong> for events to fire.</li>
  <li>Runs started by a AACFlow Workspace Events trigger never emit workspace events, so side-effect workflows cannot chain or loop.</li>
...
<FAQ items={[
  { question: "Why isn't my AACFlow Workspace Events trigger firing?", answer: "The workflow containing the trigger must be deployed — events only fire for deployed workflows. Also note the trigger never fires for the workflow it lives in, and runs started by another AACFlow Workspace Events trigger don't emit events." },
  { question: "Can a workflow started by a AACFlow Workspace Events trigger fire other AACFlow Workspace Events triggers?", answer: "No. Runs started by a AACFlow Workspace Events trigger never emit workspace events. This prevents chains and infinite loops — an alert workflow that fails can't re-trigger itself." },
