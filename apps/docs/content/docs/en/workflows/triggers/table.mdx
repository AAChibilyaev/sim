---
title: Table
description: The Table trigger runs a workflow when a row is inserted or updated in a AACFlow table.
pageType: reference
---

import { BlockPreview } from '@/components/workflow-preview'

The **Table trigger** runs a workflow when a row is inserted or updated in a [AACFlow table](/tables). Use it to react to data changes — enrich a row when it's added, or send a follow-up when a status column flips.
