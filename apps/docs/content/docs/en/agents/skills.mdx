...
Click **+ Add to AACFlow** to open the **Add Skill** dialog. The **Create** tab takes three fields:
...
<FAQ items={[
  ...
  { question: "Do skills work with all LLM providers?", answer: "Yes. The load_skill mechanism uses standard tool-calling, which is supported by all LLM providers in AACFlow. No provider-specific configuration is needed. The skill system works the same way whether you are using Anthropic, OpenAI, Google, or any other supported provider." },
