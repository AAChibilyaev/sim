...
An **agent** is a [workflow](/workflows) that reasons and acts on its own. It reads an input, decides what to do, and carries it out, calling tools and using your data along the way. You build a custom agent in AACFlow by composing a workflow whose thinking runs through one or more [Agent blocks](/workflows/blocks/agent).
