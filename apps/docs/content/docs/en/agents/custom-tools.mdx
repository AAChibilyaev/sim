...
Custom tools let you write your own JavaScript functions and make them available as callable tools in Agent blocks. This is useful when you need functionality that isn't covered by AACFlow's built-in integrations — for example, calling an internal API, performing a custom calculation, or transforming data in a specific way.
