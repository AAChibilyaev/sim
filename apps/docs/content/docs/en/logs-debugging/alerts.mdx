...
Each delivery is signed so you can confirm it came from AACFlow. The signature is in the `sim-signature` header:
...
AACFlow also sends `sim-event`, `sim-timestamp`, and an `Idempotency-Key` header on each delivery, so a receiver can dedupe retries.

## Retries

If your endpoint does not return a 2xx, AACFlow retries up to 5 times with increasing delays of 5s, 15s, 60s, 3m, then 10m, each with a little jitter. After the fifth failure the delivery is marked failed.
