---
title: Introduction
description: AACFlow is the open-source AI workspace where teams build, deploy, and manage AI agents.
---
...
When you open AACFlow, the first thing you meet is **Mothership**, a chat box over your whole workspace. You describe the system you want, and Mothership scaffolds it: workflows, tables, knowledge bases, and the wiring between them. You then open what it built, run it, and refine it.
...
A AACFlow system is not a single chat response. It is a set of resources that live in a **workspace** and connect to each other. The sidebar mirrors that anatomy, and these are its parts.
...
AACFlow provides native integrations with 1,000+ services:
...
- **Cloud-hosted.** Launch immediately at [aacflow.io](https://aacflow.io) with managed infrastructure, scaling, and observability.
...
<FAQ items={[
  { question: "Is AACFlow free to use?", answer: "AACFlow offers a free Community plan with 1,000 one-time credits to get started. Paid plans start at $25/month (Pro) with 5,000 credits and go up to $100/month (Max) with 20,000 credits. Annual billing is available at a 15% discount. You can also self-host AACFlow for free on your own infrastructure." },
  { question: "Is AACFlow open source?", answer: "Yes. AACFlow is open source under the Apache 2.0 license. The full source code is available on GitHub and you can self-host it, contribute to development, or modify it for your own needs. Enterprise features (SSO, access control) have a separate license that requires a subscription for production use." },
  { question: "Which AI models and providers are supported?", answer: "AACFlow supports 15+ providers including OpenAI, Anthropic, Google Gemini, Groq, Cerebras, DeepSeek, Mistral, xAI, and OpenRouter. You can also run local models through Ollama or VLLM at no API cost. Bring Your Own Key (BYOK) is supported so you can use your own API keys at base provider pricing with no markup." },
  { question: "Do I need coding experience to use AACFlow?", answer: "No. AACFlow lets you build agents conversationally through Mothership using natural language, or visually by connecting blocks in the workflow builder. For advanced use cases, the Function block lets you write custom JavaScript, and the full API/SDK is available for programmatic access." },
  { question: "Can I self-host AACFlow?", answer: "Yes. AACFlow provides Docker Compose configurations for self-hosted deployments. The stack includes the AACFlow application, a PostgreSQL database with pgvector, and a realtime collaboration server. You can also integrate local AI models via Ollama for a fully offline setup." },
