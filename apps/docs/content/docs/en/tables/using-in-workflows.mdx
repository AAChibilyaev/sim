...
Every row carries three built-in columns alongside your own: `id` (the row's unique identifier), `createdAt`, and `updatedAt`. AACFlow manages these for you, so you never include them when inserting. You can still filter and sort on them.
...
The output is the inserted row, including the `id` and timestamps AACFlow generated for it.
