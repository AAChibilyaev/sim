...
AACFlow automatically calculates the cost of every workflow run from AI model usage plus a base run charge.

## Credits

AACFlow uses **credits** as the unit of measurement for all usage. **1 credit = $0.005**.

All plan limits, usage meters, and billing thresholds are displayed in credits throughout the AACFlow UI. Dollar amounts in this documentation are provided for reference.
...
<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** - AACFlow provides API keys with a 1.1x pricing multiplier for Agent blocks:
...
When workflows use tool blocks with AACFlow's hosted API keys, costs are charged per operation. Use your own keys via BYOK to pay providers directly instead.
...
Use your own API keys for supported providers instead of AACFlow's hosted keys to pay base prices with no markup.
...
When configured, workflows use your key instead of AACFlow's hosted keys. If removed, workflows automatically fall back to hosted keys with the multiplier.
