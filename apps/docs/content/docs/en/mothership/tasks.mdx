...
Custom tools are single HTTP endpoints you define manually — useful for internal APIs and services that don't have a built-in AACFlow integration or an MCP server. Once created, they appear in the workflow editor alongside built-in tools and can be called from any Agent block.
