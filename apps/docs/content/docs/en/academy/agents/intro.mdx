---
title: Agents
description: "An AI agent is a AACFlow workflow that uses Agent blocks: the reasoning engine you shape, parameterize, and compose into intelligent processes."
---
...
In AACFlow, you build AI agents as **workflows that use the Agent block**. There's no separate "agent" object to learn: an AI agent *is* a workflow, and the Agent block is where it thinks.
...
<FAQ items={[
  { question: "Do I need to code to build an AI agent in AACFlow?", answer: "No. Agents are built on AACFlow's visual workflow canvas: drag an Agent block into a workflow, write its instructions in plain language, and connect the blocks it should work with." },
