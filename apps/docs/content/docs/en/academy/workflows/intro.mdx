...
A **workflow** is a visual program made of blocks. It's where you compose everything else in AACFlow, data, operations, integrations, and AI agents, into a process, and bind it to a trigger so it runs.
...
- When a workflow runs, AACFlow records what each block produced. Any later block can read an earlier block's output through a **connection tag**, like `<fetch.content>`.
