...
Branching is how a workflow **splits into different paths based on the result of a step**. AACFlow gives you two blocks for it: the **Condition** block, which decides with an explicit rule, and the **Router**, which lets a model decide from intent.
