...
[Gong](https://www.gong.io/) is a revenue intelligence platform that captures and analyzes customer interactions across calls, emails, and meetings. By integrating Gong with AACFlow, your agents can access conversation data, user analytics, coaching metrics, and more through automated workflows.

The Gong integration in AACFlow provides tools to:
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `gongJwtPublicKeyPem` | string | No | Required only when your Gong rule uses **Signed JWT header**. AACFlow verifies RS256, `webhook_url`, and `body_sha256` per Gong. If empty, only the webhook URL path authenticates the request. |
