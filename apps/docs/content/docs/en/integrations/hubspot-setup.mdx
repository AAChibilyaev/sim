...
description: Install, configure, use, and disconnect the AACFlow HubSpot integration.
pageType: guide
---
...
AACFlow connects your HubSpot CRM to AI workflows. Once connected, your workflows can:
...
- A [AACFlow](https://aacflow.io) account and a workspace where you have **Write** or **Admin** permission.
...
1. Log in to AACFlow, open your workspace, and click **Integrations** in the sidebar.
2. Search for **HubSpot** and open it.

<Image
  src="/static/integrations/hubspot/integrations-page.png"
  alt="The Integrations page in AACFlow, with the sidebar entry, a search box, connected accounts, and featured integrations"
  width={800}
  height={664}
/>

3. The HubSpot page lists its skills and templates. Click **+ Add to AACFlow** in the top right.

<Image
  src="/static/integrations/hubspot/hubspot-page-add-to-sim.png"
  alt="The HubSpot integration page in AACFlow with the Add to AACFlow button in the top right"
  width={800}
  height={550}
/>
...
<Image
  ...
  alt="HubSpot's screen for connecting your AACFlow account, with sign-in options"
  width={600}
  height={494}
/>
...
{/* VISUAL: screenshot of the HubSpot scope approval screen for the AACFlow app. */}

7. You are returned to AACFlow. The connection appears under **Connected** on the Integrations page.
...
The HubSpot block can also act as a trigger. Toggle **Use as Trigger**, select the connection, and choose which events to watch — contacts, companies, deals, or tickets, created or updated. AACFlow polls HubSpot for changes and runs the workflow once per changed record, with the record's fields available to downstream blocks.
...
## Disconnect HubSpot from AACFlow

<Callout type="warn">
When you disconnect, workflows that use this HubSpot connection will fail at the HubSpot block on their next run, and HubSpot triggers using it stop firing. Data already written to HubSpot or stored in AACFlow is not affected.
</Callout>

1. In AACFlow, click **Integrations** in the sidebar.
...
You can also remove AACFlow from the HubSpot side, which revokes its access entirely:

1. In HubSpot, go to **Settings → Integrations → Connected apps**.
2. Find **AACFlow** and choose **Uninstall**.

See HubSpot's guide to [connecting and uninstalling apps](https://knowledge.hubspot.com/integrations/connect-apps-to-hubspot) for details. Uninstalling revokes AACFlow's tokens, so connected workflows fail at the HubSpot block until you reinstall and reconnect. Your HubSpot data itself is not changed or deleted.
...
Need help? Contact [help@aacflow.io](mailto:help@aacflow.io).
