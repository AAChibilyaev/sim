...
description: Set up a Cal.com API key to use Cal.com in AACFlow workflows without OAuth
---
...
<Callout type="warn">
The API key carries the full privileges of the user who created it — there is no scope selection. Treat it like a password: do not commit it to source control or share it publicly. AACFlow encrypts the key at rest.
</Callout>
...
<Steps>
  ...
  <Step>
    Search for "Cal.com Service Account" and click it, then click **Add to AACFlow** and choose **Add API key**
  ...
  <Step>
    Click **Add API key**. AACFlow verifies the key by calling Cal.com's `/v2/me` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "Do Cal.com API keys have scopes?", answer: "No — a key carries the full permissions of the user who created it. There's nothing to select at creation time, and no Cal.com block in AACFlow can fail for missing scopes. Permission errors only arise from resource ownership, e.g. confirming a booking the key's user doesn't host." },
  { question: "What happens when the key expires?", answer: "Workflows using the credential start failing with authentication errors. Create a new key from the same Cal.com account, update the credential in AACFlow, and delete the expired key. If you couldn't create a non-expiring key, set a reminder before the expiry date you chose." },
  ...
  { question: "Does this work with self-hosted Cal.com?", answer: "No — AACFlow's Cal.com blocks call the cal.com cloud API (api.cal.com). Self-hosted Cal.com instances expose their own API host and are not supported by this credential." },
  { question: "How do I rotate the key?", answer: "Create a new key from Settings → Developer → API keys, update the credential in AACFlow with the new key, and once it's working, delete the old one in Cal.com." },
]} />
