...
description: Set up a monday.com personal API token on a dedicated integration user to use monday.com in AACFlow workflows
---
...
<Steps>
  ...
  <Step>
    Search for "monday.com Service Account" and click it, then click **Add to AACFlow** and choose **Add API token**
  ...
  <Step>
    Click **Add API token**. AACFlow verifies the token by querying monday.com's `me` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "How do I rotate the token?", answer: "Carefully. monday.com allows only one token per user, and regenerating immediately kills the old one — there's no dual-token window. Regenerate in monday.com, then update the credential in AACFlow right away. Expect a brief outage for any workflows that run in between." },
