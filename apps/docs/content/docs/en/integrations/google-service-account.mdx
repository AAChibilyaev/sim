...
Before adding a service account to AACFlow, you need to configure it in the Google Cloud Console and Google Workspace Admin Console.
...
The table below lists every Google service that supports service account authentication in AACFlow, the API to enable in Google Cloud Console, and the delegation scopes to authorize. Copy the scope string for each service you need and paste it into the Google Workspace Admin Console.
...
Once Google Cloud and Workspace are configured, add the service account as a credential in AACFlow.
