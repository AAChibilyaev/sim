...
The A2A (Agent-to-Agent) protocol lets AACFlow call external AI agents that expose an A2A-compatible endpoint. Use it to connect your workflows to remote agents — LLM-powered bots, microservices, and other AI systems — through a standardized message format.
...
- Task IDs are scoped to the external agent, not to AACFlow. Anyone who knows an agent URL and task ID can read or cancel that task unless the agent enforces its own authentication — set an API key for agents that require one.
