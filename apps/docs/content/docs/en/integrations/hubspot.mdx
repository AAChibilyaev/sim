...
With the HubSpot integration in AACFlow, you can:
...
In AACFlow, the HubSpot integration enables your agents to interact with your CRM data as part of automated workflows. Agents can qualify leads, enrich contact records, track deals, and synchronize data across your tech stack—enabling intelligent sales and marketing automation.
...
| Parameter | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `triggerCredentials` | string | Yes | Connect a HubSpot account so AACFlow can poll your CRM on your behalf. |
...
| `properties` | string | No | Properties to include on each record. Leave empty to use sensible defaults. AACFlow always includes the timestamps it needs internally. |
...
| `timestamp` | string | ISO timestamp when AACFlow emitted the event |
