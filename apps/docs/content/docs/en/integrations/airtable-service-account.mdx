...
description: Set up an Airtable personal access token with scoped base access to use Airtable in AACFlow workflows
---
...
<Steps>
  ...
  <Step>
    Add the scopes your workflows need. The full set AACFlow's Airtable tools and triggers use is:
    ...
    Reading records needs `data.records:read`; creating, updating, upserting, and deleting records need `data.records:write`; listing bases and tables and reading base schemas need `schema.bases:read`; `user.email:read` lets AACFlow show the token owner's email as the credential name; and `webhook:manage` is only needed if you use Airtable triggers in AACFlow.
...
<Steps>
  ...
  <Step>
    Search for "Airtable Service Account" and click it, then click **Add to AACFlow** and choose **Add personal access token**
  ...
  <Step>
    Click **Add personal access token**. AACFlow verifies the token by calling Airtable's `whoami` endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "How do I rotate the token?", answer: "Create a new token with the same scopes and base access, update the credential in AACFlow with the new token, and once it's working, delete the old one. Note that regenerating an existing token invalidates the old value immediately." },
  { question: "Do I need the webhook:manage scope?", answer: "Only if you use Airtable triggers in AACFlow — the trigger creates and manages webhooks on your base. For workflows that only read and write records, you can omit it." },
]} />
