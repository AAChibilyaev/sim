...
description: Connect Pipedrive to AACFlow with a personal API token instead of an OAuth login
---
...
<Callout type="warn">
The API token grants everything its user can see and do in Pipedrive. Treat it like a password — do not commit it to source control or share it publicly. AACFlow encrypts the token at rest.
</Callout>

## Adding the API Token to AACFlow
...
<Steps>
  ...
  <Step>
    Search for "Pipedrive" and open it, then click **Add to AACFlow** and choose **Add API token**
  </Step>
  <Step>
    Paste the **API token**, and optionally set a display name and description
  </Step>
  <Step>
    Click **Add API token**. AACFlow verifies the token against Pipedrive (`GET /v1/users/me`) and names the credential after the Pipedrive user and company — if verification fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
AACFlow sends the token in Pipedrive's `x-api-token` header (the documented scheme for personal API tokens), so every Pipedrive tool works unchanged.
...
Pipedrive tokens don't expire on a schedule. To rotate one, regenerate it on the same **Personal preferences** → **API** page, then paste the new value into the credential in AACFlow right away — the old token stops working the moment a new one is generated.
...
<FAQ items={[
  ...
  { question: "What happens if the user who owns the token is deactivated?", answer: "The token stops working immediately. Issue a new token from an active user (ideally a dedicated integration user) and update the credential in AACFlow." },
