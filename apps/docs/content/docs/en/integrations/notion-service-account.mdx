...
description: Set up a Notion internal integration with an integration secret to use Notion in AACFlow workflows
---
...
<Callout type="warn">
The secret is bearer credentials for everything the integration is connected to. Treat it like a password — do not commit it to source control or share it publicly. AACFlow encrypts the secret at rest.
</Callout>
...
<Callout type="warn">
A valid secret with no page connections validates fine in AACFlow but returns `404 object_not_found` on every real call. If a Notion block reports that a page or database wasn't found — and you're sure the ID is right — the page isn't connected to the integration.
</Callout>
...
<Steps>
  ...
  <Step>
    Search for "Notion Service Account" and click it, then click **Add to AACFlow** and choose **Add integration secret**
  ...
  <Step>
    Click **Add integration secret**. AACFlow verifies the secret by calling Notion's bot-user endpoint — if it fails, you'll see a specific error explaining what went wrong.
  </Step>
</Steps>
...
<FAQ items={[
  ...
  { question: "My secret starts with secret_ instead of ntn_ — is that a problem?", answer: "No. Notion switched new secrets to the ntn_ prefix in September 2024, but existing secret_ tokens remain valid. AACFlow accepts both." },
  ...
  { question: "How do I rotate the secret?", answer: "The integration's Configuration tab has a refresh/regenerate action for the secret. Regenerate it, then update the credential in AACFlow with the new value. Plan for the old secret to stop working as soon as you regenerate." },
