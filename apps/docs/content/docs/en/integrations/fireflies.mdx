...
[Fireflies.ai](https://fireflies.ai/) is a meeting transcription and intelligence platform that integrates with AACFlow, allowing your agents to work directly with meeting recordings, transcripts, and insights through no-code automations.

The Fireflies integration in AACFlow provides tools to:
...
- **Trigger workflows on transcription completion:** Activate AACFlow workflows automatically when a Fireflies meeting transcription finishes using the provided webhook trigger—enabling real-time automations and notifications based on new meeting data.
